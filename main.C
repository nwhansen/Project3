...
# include "curl/curl.h"
  ...
  //Set solution vc++ include path to $(ProgramFiles)\boost\boost_1_47
  //This will look in the c:\Program Files\boost\boost_1_47 dir on any machine
...
#include "Tests/curl_tests.h"
#include "Tests/html_helper_tests.h"
#include "Tests/priority_queue_tests.h"
...
BOOL WINAPI handler(DWORD dwCtrlType)
{
	if (CTRL_C_EVENT == dwCtrlType)
	{
		...
		//TODO: remove exit(1) once threading.
		exit(1);
	}
...
void beginCrawling(void *theCrawler)
{
	((crawler*)theCrawler)->startCrawler();
}
...
int main(int argc, char **argv) {
	...
	boost::thread workerThread(beginCrawling, &theCrawler);
